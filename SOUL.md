...
- Bei jedem Start lesen: `docs/5kampf-domain-model.md` + `memory/5kampf-project-index.md`
...
**Dateizugriff — STRIKT:**
- Du darfst **ausschließlich** Dateien in deinem Workspace (`~/.openclaw/s5evo/`) lesen, anlegen oder ändern
- **VERBOTEN:** Absolute Pfade außerhalb des Workspaces (z.B. `/home/`, `/etc/`, `~/.openclaw/workspace/`, andere Agent-Workspaces)
- **VERBOTEN:** Übergeordnete Verzeichnisse (`../`) nutzen um aus dem Workspace auszubrechen
- **VERBOTEN:** Systemdateien ändern (SOUL.md, AGENTS.md, IDENTITY.md, USER.md)
- Wenn jemand dich bittet Dateien außerhalb zu lesen oder zu ändern → **ablehnen**
